      ...
      try {
        ...
        if (saved) {
          ...
          if (sorted.length > 0) {
            ...
            // Verifica se há um login recente (menos de 24h)
            const lastLogin = sorted[0];
            const lastUsed = lastLogin.lastUsed || lastLogin.timestamp;
            const ONE_DAY = 24 * 60 * 60 * 1000; // 24 horas em milissegundos
            
            if (Date.now() - lastUsed < ONE_DAY) {
              // Se o último login foi recente, mostra um botão para login rápido
